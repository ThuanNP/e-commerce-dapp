...
export class PurchaseContractEffects {
      ...
      exhaustMap((result) =>
        concat(
          ...
          this.purchaseSrv.confirmPurchase(result.address, result.eth).pipe(
            tap((address) =>
              console.log(
                `Purchase confirmed successfully for the contract: ${address} `
              )
            ),
            concatMapTo([
  ...
  showSpinner$ = createEffect(() =>
    this.actions$.pipe(
      ofType(
        PurchaseContractActions.createPurchaseContract,
