...
import { EthereumMetamaskProviderToken } from './tokens';

@Injectable({ providedIn: 'root' })
export class EthersWeb3ProviderService {
  constructor(@Inject(EthereumMetamaskProviderToken) private ethProvider) {
  ...
  public getSelectedAddress(): Observable<string> {
    const web3Provider: ethers.providers.JsonRpcProvider = new ethers.providers.Web3Provider(
      this.ethProvider
    );
