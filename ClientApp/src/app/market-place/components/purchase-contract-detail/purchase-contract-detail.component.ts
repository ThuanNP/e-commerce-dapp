...
export class PurchaseContractDetailComponent implements OnChanges {
  ...
  buyerConfirmPrice = new FormControl('', [
    Validators.required,
    ,
    Validators.pattern(/^\d+(\.\d{1,4})?$/),
  ]);

  constructor(@Inject(windowRefToken) private windowRef: Window) {}

  ngOnChanges(changes: SimpleChanges) {
    if (this.imageRef) {
      this.imageRef.nativeElement.src = this.windowRef.window.URL.createObjectURL(
        this.image
      );
    }
  ...
  get validatorError() {
    return this.buyerConfirmPrice.hasError('required')
      ? 'You must enter a value'
      : this.buyerConfirmPrice.hasError('pattern')
      ? 'Not a valid format'
      : '';
  }
  ...
  get removeFromListing() {
    return (
      this.contract.state === ContractState.Canceled &&
  ...
  */
  ...
  get canDelivery() {
    return (
      this.contract.state === ContractState.Locked &&
  ...
  get canReleaseEscrow() {
    return (
      (this.contract.state === ContractState.ItemReceived ||
        this.contract.state === ContractState.OwnerPaid) &&
      this.contract.sellerAddress === this.account
    );
  }
  ...
  get canReceiveCommission() {
    return (
      (this.contract.state === ContractState.ItemReceived ||
        this.contract.state === ContractState.SellerPaid) &&
      this.contract.ownerAddress === this.account
    );
  }
}
